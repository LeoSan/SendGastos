...
class Gastos_Model_Cuentas 
{
	...
	{
		$query = "SELECT * FROM ( (SELECT   FA.IDGASTOMAIN GASTO, FA.FCCREDITOCARTERA CREDITO, TO_CHAR(FA.FDFECREGISTRO,'DD-MON-YY', 'nls_date_language=Spanish') FECHAREGISTRO,
         ...
         (CASE
            ...
            ELSE 'EN SOLUCION'
         END) STATUS, TO_CHAR(FDFECREALPAGO,'DD-MON-YY', 'nls_date_language=Spanish') FDFECREALPAGO, FCREMESA, TO_CHAR(FDFECSERVPAGADODEL,'DD-MON-YY', 'nls_date_language=Spanish') FDFECSERVPAGADODEL,
		 TO_CHAR(FDFECSERVPAGADOAL,'DD-MON-YY', 'nls_date_language=Spanish') FDFECSERVPAGADOAL,  CASE WHEN FCPAGADOPREVIAMENTE = 'S' THEN 'SI' ELSE 'NO' END FCPAGADOPREVIAMENTE, 
	...
	INNER JOIN
        ...
        WHERE FCCREDITOCARTERA = '$credito' AND FA.STATUS = 'A' AND FM.FCSTATUS != 'Z'  AND FM.FCSTATUS != 'R'  AND FB.IDTASKGASTO = '4515947455273e63c4198f0073790158' AND FB.FCRESULTADO = 'Autorizado'
         ...
         INNER JOIN PENDUPM.FACTURACIONBITACORA FB
           ON (FA.IDGASTOMAIN = FB.IDGASTOMAIN)
        WHERE FCCREDITOCARTERA = '$credito' AND FA.STATUS = 'A' AND FM.FCSTATUS != 'Z') ) ORDER BY GASTO DESC ";
	...
	public function getDocumentsByCase($idGasto,$credito,$idConcepto,$idcomprobacion=0 )
	{
		$query = "(SELECT FCCONCEPTO CONCEPTO,'Comprobacion' TIPO,FCARCHIVOPDF PDF, FCARCHIVOXML XML FROM PENDUPM.FACTURACIONCOMPROBA WHERE IDGASTOMAIN = $idGasto)
			UNION
			(SELECT FCNOMBRE CONCEPTO,'Anexos' TIPO,FCRUTAFILE PDF, '' XML FROM PENDUPM.FACTURACIONANEXOS WHERE IDGASTOMAIN = $idGasto)
			UNION
			(SELECT FCTIPOCOTIZACION CONCEPTO,FCTIPOCOTIZACION TIPO,FCARCHIVOPDF PDF, FCARCHIVOXML XML FROM PENDUPM.FACTURACIONCOTIZA WHERE IDGASTOMAIN = $idGasto)
			UNION
		...
		if ($idcomprobacion==0) {
			$query = "SELECT FCCONCEPTO CONCEPTO,FCTIPOCOMPROBANTE TIPO,FCARCHIVOPDF PDF, FCARCHIVOXML XML, FCARCHIVOPDFC PDFC
			...
			$query = "SELECT FCCONCEPTO CONCEPTO,FCTIPOCOMPROBANTE TIPO,FCARCHIVOPDF PDF, FCARCHIVOXML XML , FCARCHIVOPDFC PDFC
                      FROM PENDUPM.FACTURACIONCOMPROBA F INNER JOIN PENDUPM.FACTURAASIGNACION A ON F.IDCOMPROBACION = A.IDCOMPROBACION 
			 WHERE A.IDGASTOMAIN = $idGasto AND A.FCCREDITOCARTERA = '$credito' AND IDCONCEPTO = $idConcepto AND F.IDCOMPROBACION=$idcomprobacion";
		}
	...
	{
		$query = "SELECT COUNT(1) TOTAL FROM PENDUPM.CTCATALOGOCUENTAS WHERE IDCONCEPTO = $concepto "; 
	...
	public function setFechasPagoServicios($params) {
		$query = "BEGIN UPDATE PENDUPM.FACTURAASIGNACION SET FDFECSERVPAGADODEL = TO_DATE('".$params["FDFECSERVPAGADODEL"]."','DD/MM/YY'), FDFECSERVPAGADOAL = TO_DATE('".$params["FDFECSERVPAGADOAL"]."','DD/MM/YY') 
		          WHERE IDGASTOMAIN = ".$params["idgasto"]." AND IDCONCEPTO = ".$params["concepto"]." 
		                AND FCCREDITOCARTERA = '".$params["credito"]."' ; 
		 COMMIT;
		 END;"; 
