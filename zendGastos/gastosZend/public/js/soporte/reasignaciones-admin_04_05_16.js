var SoportesReasignaciones = {
			...
			$("#idUserSearchAdmin").keypress(function (e) {
				if (e.which == 13) {
					...
					SoportesReasignaciones.searchData();
						...
						var name=$.trim($("#idUserSearchAdmin").val());
						if(SoportesReasignaciones.bnd==false && name.length>0){
							SoportesReasignaciones.bnd=true;
							...
							$.ajax({
								  ...
								  dataType: 'html',
								  complete: function() {
								  	  SoportesReasignaciones.bnd=false;
								  },
								  success: function(data) {
									  ...
									  else $('#idUserSearchAdmin').typeahead({source: arrdata,minLength: 0});
								  }
							});
						}
				}
		...
		initTabla : function() {
			...
			/*$("#newuser").off('keypress').on("keypress", function(e){
				...
				SoportesReasignaciones.findUserAdmin();
			});*/
