var ReporteDiario = {
		init : function() {
                        $(".grid-title").html("Propuesta de pago servicios CIE");
...
//							console.log($("#tablaFechas").children().first().children().first().children().first().html());
							...
							$.ajax({
						        type: "POST",
						        dataType: 'html',
						        url: baseUrl + '/catalogos/contabilidad/getformatopagocie',
						        data: {
		...
		checkAllFields : function() {
			...
			if($( "#fechaReporte" ).val() != "" &&
				...
				$.ajax({
			        type: "POST",
			        dataType: 'html',
			        url: baseUrl + '/catalogos/contabilidad/getfoliospagoscie',
			        data: {
						fecha: $("#fechaReporte").val(),
