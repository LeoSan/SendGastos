var SoportePlanViajeReasignaciones = {
		init : function() {
			$(".grid-title").html("Reasignaciones de planes de viaje");
			...
			*/
			$( document ).on({
				click : function( e ) {
					...
					$.ajax({
						type: "POST",
						url: baseUrl + '/catalogos/soporteplandeviaje/getdatoscomplementarios',
						data: { gastoid: idGasto },
						dataType: 'json',
						success: function(data) {
							console.log(data);
							if(data.success == true){
								$("#usruid").val( data.usruid );
								$("#appuid").val( data.appuid );
								$("#delindex").val( data.delindex );
								$("#responsableid").val( data.currentusr );
							} else {
			...
			$("#btnGuardarReasignacion").click(function(e){
				...
				$.ajax({
			        type: "POST",
			        dataType: 'json',
			        url: baseUrl + '/catalogos/soporteplandeviaje/reasignar',
			        data: {
			        	gastoid: 		gastoid,
			        	...
			        	*/
			        	if (data.success == true) {
			        		$.ajax({
						        type: "POST",
						        dataType: 'json',
						        url: baseUrl + '/catalogos/soporteplandeviaje/actualizareasignacion',
						        data: {
						        	usr_uid: 	$("#usruid").val(),
						        	app_uid: 	$("#appuid").val(),
...
//						        	return false;
						        	if (data.success == true) {
						        		...
						        		Menu.loadModuleMenu('catalogos', 'soporteplandeviaje', 'reasignaciones', '#');
						        		Layout.hideLoading();
						            } else {
		...
		contarSeleccionados : function() {
			...
			var cadena = '';
			var cadenaArray = '';
			$("#tablaSoportePlanViajeReasignaciones input[type=checkbox]").each(function(e){
...
SoportePlanViajeReasignaciones.init();